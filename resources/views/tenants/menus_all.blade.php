...
<div class="absolute bottom-0 top-28 left-44">
    <div class="container mx-auto w-screen max-w-6xl mt-8 flex">
        <!-- Kolom Menu -->
        <div class="w-2/3 pr-4">
            <a href="{{ route('kasir.index') }}" class="bg-blue-500 text-white px-4 py-2 rounded mb-4 inline-block">
                Kembali
            </a>
            <h1 class="text-2xl font-bold mb-4 text-center">All Menus for {{ $tenant->namatenant }}</h1>
            <!-- Search Bar -->
            ...
            <div class="flex flex-wrap gap-4 mb-8">
                @foreach($menus as $menu)
                    <div class="bg-white shadow-md rounded-lg overflow-hidden w-56 menu-card cursor-pointer"
                        data-menu-id="{{ $menu->id }}"
                        data-tenant-id="{{ $tenant->id }}"
                        data-menu-price="{{ $menu->price }}"
                        data-menu-name="{{ $menu->name }}"
                        data-tenant-name="{{ $tenant->namatenant }}">
                        <img class="w-full h-32 object-cover" src="{{ asset('img/contoh.png') }}" alt="{{ $menu->name }}">
